...
最初に重要な前提を置きます。この表は「完全互換表」ではありません。Claude Code、Codex CLI、OpenCode、Aider、Antigravity CLI は、どれも AI coding agent の文脈で語られますが、設計思想がかなり違います。Claude Code と Codex CLI は agentic CLI としてかなり近いカテゴリにあります。Antigravity CLI の `agy` も、`-c`、`--conversation`、`-p`、`--model`、`--dangerously-skip-permissions` などを見る限り、かなり Claude Code に近い操作体系です。OpenCode も同じく terminal-first の agent ですが、モデルプロバイダ非依存、agent 設定、OpenCode 独自の TUI/サーバー構成が前面にあります。Aider はより古典的な AI pair programming tool で、明示的にファイルを chat に追加し、diff と git commit を中心に作業する思想です。
...
- **確認できず**: `--help` や手元の実測では同じ機能を確認できなかった操作です。
...
Antigravity CLI については、この文書では `agy` コマンドだけを対象にします。Antigravity にはエディタを開くための `antigravity` コマンドもありますが、これは VS Code 系の editor launcher に近く、Claude Code のような agent CLI の比較対象としては `agy` の方が適切です。ここでは手元の `agy --help`、`agy models --help`、`agy plugin --help`、`agy install --help`、`agy changelog --help` で確認できた範囲を反映します。
...
| --- | --- | --- | --- | --- | --- |
| 対話セッション開始 | `claude` | `codex` | `opencode` | `aider` | `agy` |
| 初回プロンプト付き起動 | `claude "..."` | `codex "..."` | `opencode --prompt "..."` | `aider -m "..."` または起動後に入力 | `agy --prompt-interactive "..."` |
| プロジェクト初期化 | `/init` | `/init` | `/init` | 対応なし。手動で `.aider.conf.yml` 等を作る | `/init` 相当は確認できず |
| 指示ファイル | `CLAUDE.md` | `AGENTS.md` | `AGENTS.md` | `.aider.conf.yml`、`.env`、慣習ファイル等 | 確認できず |
| 直近セッション継続 | `claude -c` | `codex resume` | `opencode -c` | 近似: `--restore-chat-history` | `agy -c` / `agy --continue` |
| 特定セッション再開 | `claude -r <session>` | `codex resume <session>` | `opencode -s <sessionID>` | 対応なしに近い | `agy --conversation <id>` |
| 非対話実行 | `claude -p "..."` | `codex exec "..."` | `opencode run "..."` | `aider --message "..."` | `agy -p "..."` / `agy --print "..."` |
| JSON 出力 | `claude -p --output-format json` | `codex exec --json "..."` | `opencode run --format json "..."` | 対応なしに近い | 確認できず |
| モデル指定 | `claude --model ...` | `codex -m ...` / `codex exec -m ...` | `opencode -m provider/model` | `aider --model ...` | `agy --model ...` |
| モデル一覧 | モデル選択 UI / docs | `codex debug models` | `opencode models` | `aider --list-models ...` | `agy models` |
| 実行中モデル切替 | `/model` | `/model` | TUI/設定でモデル指定、`--model` | `/model` | 起動時 `--model` は確認。実行中 slash は確認できず |
| 計画モード | `--permission-mode plan` など | `/plan` | `Tab` で `plan` agent、または `--agent plan` | `/ask`、`/architect` が近い | 確認できず |
| ファイル参照 | パス指定、mention | `/mention` | `@file`、`--file` | `aider file.py`、`/add`、`/read-only` | `--add-dir` で workspace 追加。ファイル mention は確認できず |
| 差分確認 | UI / git diff | `/diff` | `/undo` 前に差分確認、通常は git diff 併用 | `/diff` | 確認できず |
| 取り消し | escape/checkpoint 系、git 復元 | 変更は git / diff 前提、`/archive` 等は会話管理 | `/undo`、`/redo` | `/undo` は aider commit を戻す | 確認できず |
| MCP 管理 | `claude mcp` | `codex mcp`、`/mcp` | `opencode mcp` | 対応なしに近い | 確認できず |
| plugin 管理 | Skills / plugins | `codex plugin` | `opencode plugin` | 対応なしに近い | `agy plugin ...` |
| 権限スキップ | `--dangerously-skip-permissions` | `--dangerously-bypass-approvals-and-sandbox` | `--dangerously-skip-permissions` | `--yes-always` は近似 | `agy --dangerously-skip-permissions` |
| sandbox | 権限モード / settings | `--sandbox ...` | permission / sandbox 設定 | 対応なしに近い | `agy --sandbox` |
| 更新 | `claude update` | `codex update` | `opencode upgrade` | `aider --upgrade` | `agy update` |
| 診断 | 専用 `doctor` は公式 CLI 表に確認なし | `codex doctor` | `opencode debug` | `aider --help`、`/settings` | `agy --help`、`agy changelog` |
...
Antigravity CLI は `agy` で起動します。

```bash
agy
```

初回プロンプトを渡して、そのまま対話セッションを続けたい場合は `--prompt-interactive` を使います。短縮形は `-i` です。

```bash
agy --prompt-interactive "このリポジトリの構成を説明して"
agy -i "この変更方針を相談したい"
```

ここは Claude Code とかなり近い感覚です。`claude "..."` が「最初の依頼を入れて会話を始める」操作だとすると、`agy --prompt-interactive "..."` が近い位置にあります。一方で、`agy --help` では裸の positionals に初回 prompt を渡す仕様までは確認できなかったため、この文書では明示的な `--prompt-interactive` を推奨します。
...
Antigravity CLI の `agy --help` では、Claude Code の `/init` に相当する初期化コマンドは確認できませんでした。少なくとも CLI の top-level option と subcommand としては、`install`、`models`、`plugin`、`update`、`changelog` が確認できる一方、`init` は表示されません。
...
このため、Antigravity CLI で Claude Code の `/init` と同じことをしたい場合は、「同じコマンドがある」とは書かず、「プロジェクト指示ファイルや永続ルールの扱いは別途確認が必要」とするのが安全です。`agy` はセッション実行や再開、非対話実行については Claude Code にかなり近いですが、プロジェクトを解析して `CLAUDE.md` や `AGENTS.md` を生成する入口は、手元の `agy --help` からは確認できません。
...
Antigravity CLI の `agy` は、この点では Claude Code にかなり近いです。直近の conversation を続けるには `-c` または `--continue` を使います。
...
特定の conversation を再開するには `--conversation` を使います。

```bash
agy --conversation <conversation-id>
```

Claude Code の `claude -c` に慣れているなら、Antigravity CLI では `agy -c` と覚えるのが自然です。Claude Code の `claude -r <session>` に近いのは `agy --conversation <conversation-id>` です。ただし、`agy --help` 上では conversation の一覧表示コマンドは確認できませんでした。過去 conversation の ID をどこで取得するかは、Antigravity 側の UI やローカル履歴、あるいは今後の CLI ドキュメントを確認する必要があります。
...
Antigravity CLI の非対話実行は `--print` です。短縮形は `-p` で、`--prompt` は `--print` の alias として表示されています。

```bash
agy --print "このリポジトリの構成を説明して"
agy -p "この差分のリスクを要約して"
agy --prompt "テスト失敗の原因を推測して"
```

また、print mode には timeout を指定できます。

```bash
agy -p "長いログを読んで原因を説明して" --print-timeout 10m
```

ここも Claude Code にかなり近いです。`claude -p "..."` の置き換えとして、まず `agy -p "..."` を試すのが自然です。ただし、`agy --help` では JSON 出力オプションは確認できませんでした。機械可読な event stream が必要なら、現時点では Codex CLI の `codex exec --json` や OpenCode の `opencode run --format json` の方が明示的です。
...
Antigravity CLI の `agy` でも `--model` が使えます。

```bash
agy --model <model>
agy -p "この関数をレビューして" --model <model>
```

利用可能なモデルの一覧は `agy models` で確認できます。

```bash
agy models
```

Claude Code の `claude --model ...` と対応させるなら、Antigravity CLI では `agy --model ...` です。実行中に slash command として `/model` が使えるかどうかは、手元の `agy --help` からは確認できませんでした。そのため、この文書では起動時または非対話実行時の `--model` を確認済みの対応として扱います。
...
Antigravity CLI の `agy --help` では、Claude Code の plan mode に相当する明示的な `--permission-mode plan` や `/plan` は確認できませんでした。一方で、権限と実行環境に関係する option として `--sandbox` と `--dangerously-skip-permissions` が確認できます。
...
`--sandbox` は terminal restrictions enabled の sandbox で実行する option です。`--dangerously-skip-permissions` は、tool permission request を prompt なしで auto-approve する option です。名前の通り危険な option なので、Claude Code の `--dangerously-skip-permissions` と同じく、日常運用で雑に使うべきではありません。Plan 的に「まず読むだけで方針を出す」作業をしたい場合は、現時点ではプロンプトで明示的に「編集しない」「コマンドを実行しない」「計画だけ出す」と指示するのが安全です。
...
Antigravity CLI の `agy` では、workspace に追加するディレクトリを `--add-dir` で指定できます。

```bash
agy --add-dir ../shared
agy --add-dir ../shared --add-dir ../docs
```

これは Claude Code や Codex CLI の「追加の読み書き対象ディレクトリを渡す」操作に近いです。ただし、`@file` のようなファイル mention や、Aider の `/add file.py` のような chat への明示的なファイル追加コマンドは、`agy --help` からは確認できませんでした。特定ファイルを読ませたい場合は、プロンプト内でパスを明記する運用から始めるのが無難です。
...
Antigravity CLI の `agy --help` では、`/diff`、`/undo`、`/redo` に相当する CLI option や subcommand は確認できませんでした。Antigravity 側の UI では差分確認や変更管理が提供される可能性がありますが、少なくとも `agy` の top-level help だけで Claude Code / OpenCode / Aider のような undo command を書くことはできません。実務では、`git diff`、`git status`、小さな commit を併用する前提で使うのが安全です。
...
Antigravity CLI の `agy --help` では、MCP 管理コマンドは確認できませんでした。`agy` の subcommand として確認できるのは、`changelog`、`help`、`install`、`models`、`plugin`、`plugins`、`update` です。
...
agy plugin import [source]
```

`agy plugin import` は、help 上では `gemini` または `claude` から plugin を import できる形式として表示されます。つまり、Antigravity CLI は MCP 管理よりも plugin 管理を CLI の明示的な拡張点として出しています。Claude Code の MCP と一対一対応させるのではなく、`agy plugin ...` を Antigravity 側の拡張管理コマンドとして別枠で見る方が分かりやすいです。
...
| 目的 | Claude Code | Codex CLI | OpenCode | Aider | Antigravity CLI (`agy`) |
| --- | --- | --- | --- | --- | --- |
| プロジェクト指示 | `CLAUDE.md` | `AGENTS.md` | `AGENTS.md` | 規約ファイルを chat に追加、または README 等 | `agy --help` では確認できず |
| ツール設定 | `.claude/settings...` | `~/.codex/config.toml`、project config | `opencode.jsonc` | `.aider.conf.yml` | `agy install`、Antigravity 側設定 |
| コマンド拡張 | slash commands / skills | custom prompts / skills / plugins | `.opencode/commands/*.md` | `/load`、`.aider.conf.yml`、外部スクリプト | `agy plugin ...` |
| ignore | 設定次第 | 設定次第 | 設定次第 | `.aiderignore` | `agy --help` では確認できず |
...
Antigravity CLI の `agy --help` では、ログイン専用の top-level subcommand は確認できませんでした。一方で、手元の環境では Antigravity 側の OAuth token や conversation state が Antigravity のローカル領域に保存されていました。実運用では、Antigravity desktop / agent 環境側でログインし、`agy` はその認証状態を利用するものとして扱うのが自然です。
...
Antigravity CLI の更新は `agy update` です。環境設定や PATH のセットアップには `agy install` を使います。リリースノートや変更履歴は `agy changelog` で確認できます。
...
`agy install --help` では、PATH 設定を避ける `--skip-path`、alias purge を避ける `--skip-aliases`、対象ディレクトリを指定する `--dir` が確認できます。

```bash
agy install --dir ~/.local/bin
...
Antigravity CLI は、Google Antigravity の agent CLI です。この文書では `agy` コマンドだけを比較対象にしています。`agy` は `--continue`、`--conversation`、`--print`、`--prompt-interactive`、`--model`、`--sandbox`、`--dangerously-skip-permissions` を持っており、Claude Code の CLI 体験にかなり近いです。一方で、`/init`、MCP 管理、JSON 出力、`/undo`、ファイル mention のような機能は、少なくとも `agy --help` からは確認できませんでした。したがって、Antigravity CLI は「Claude Code 風に使える部分が多いが、拡張管理は `agy plugin`、プロジェクト初期化や細かな slash command は別確認が必要な CLI」と理解するとよいです。
